...
def _render(persona: str, corpus: str, burst: bool, i: int) -> str | None:
    ...
    if corpus == "day":
        path = ROOT / "eval" / "scenarios" / "day" / persona / f"{persona}_day_{i:03d}.yaml"
        if not path.exists():
            return None
        return narrate(persona, cfg, load_scenario(path), 1)
    path = ROOT / "eval" / "scenarios" / "multiday" / persona / f"{persona}_multi_{i:03d}.yaml"
    if not path.exists():
        return None
    ...
    for corpus, burst in SUBCORPORA:
        for p in PERSONAS:
            ...
            prompt = HEADER.format(
                n=len(sids), name=DISPLAY[p], profile=profile, records="\n\n".join(blocks)
            )
            fn = f"{_tag(corpus, burst)}_{p}"
            (batches / f"{fn}.md").write_text(prompt, encoding="utf-8")
            man.append({"batch": fn, "corpus": corpus, "burst": burst, "persona": p, "scenarios": sids})
    (sdir / "manifest.yaml").write_text(
        yaml.safe_dump({"slice": s, "chunk": CHUNK, "n_batches": len(man), "batches": man}, sort_keys=False),
        encoding="utf-8",
    )
    print(f"slice {s}: built {len(man)} batches ({sum(len(b['scenarios']) for b in man)} records) -> {batches}")
...
def aggregate(s: int) -> None:
    ...
    lines.append(f"- **TOTAL: {gp}/{gt} PASS ({pct}%)**, {len(flags)} FLAG, {judged} judged, {len(missing)} batch(es) missing\n")
    for f in flags[:40]:
        lines.append(f"    - {f}\n")
...
def main() -> None:
    ...
    for a in sys.argv[1:]:
        if a.startswith("--slice"):
            s = int(a.split("=")[1]) if "=" in a else int(sys.argv[sys.argv.index(a) + 1])
