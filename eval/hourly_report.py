...
def collect() -> dict:
    slices = sorted(int(p.name.split("_")[1]) for p in BASE.glob("slice_*") if p.is_dir())
...
def render_md(d: dict) -> str:
    ...
    off = [sum(d["by_sub"][s][i] for s in SUBCORPORA if s.endswith("OFF")) for i in (0, 1)]
    on = [sum(d["by_sub"][s][i] for s in SUBCORPORA if s.endswith("ON")) for i in (0, 1)]
    L = [
    ...
    for s in SUBCORPORA:
        L.append(f"| {LABEL[s]} | {d['by_sub'][s][0]}/{d['by_sub'][s][1]} | {_pct(d['by_sub'][s])} |")
    L += ["", "## Per persona", "", "| persona | PASS | rate |", "|---|---|---|"]
    for p in sorted(d["by_persona"]):
        L.append(f"| {p} | {d['by_persona'][p][0]}/{d['by_persona'][p][1]} | {_pct(d['by_persona'][p])} |")
...
def render_html(d: dict, md: str) -> str:
    ...
    def bar(pt):
        ...
        hue = 120 * (r / 100)
        return (f'<div class="bar"><span style="width:{r:.1f}%;background:hsl({hue:.0f} 65% 45%)"></span>'
                f'<em>{_pct(pt)} · {p}/{t}</em></div>')

    rows_sub = "".join(f"<tr><td>{LABEL[s]}</td><td>{bar(d['by_sub'][s])}</td></tr>" for s in SUBCORPORA)
    rows_p = "".join(f"<tr><td>{p}</td><td>{bar(d['by_persona'][p])}</td></tr>" for p in sorted(d["by_persona"]))
    flag_rows = "".join(
